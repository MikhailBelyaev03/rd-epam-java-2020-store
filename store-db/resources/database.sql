...
create table IF NOT EXISTS store.st_product (
  id uuid primary key,
  name varchar,
  description text,
...
create table IF NOT EXISTS store.st_catalog (
  id uuid primary key,
  product_id uuid,
  quantity int8,
...
create table IF NOT EXISTS store.st_supplier_order(
  id uuid primary key,
  amount int8,
  status varchar,
...
create table IF NOT EXISTS store.st_supplier_order_items (
  id uuid primary key,
  product_id uuid,
  order_id uuid,
  quantity int8,
...
create table IF NOT EXISTS store.st_client_order (
  id uuid primary key,
  amount int8,
  status varchar,
  payment_id uuid
...
create table IF NOT EXISTS store.st_client_order_items (
  id uuid primary key,
  product_id uuid,
  order_id uuid,
  quantity int8,
...
create table IF NOT EXISTS store.st_payment (
  id uuid primary key,
  ogrn_shop varchar,
  kpp_shop varchar,
  inn_shop varchar,
